...
# 'hello', 'Azuma', 0.2, 0.5, 0.9, 1
def tts_fn(text, speaker = 'Azuma', sdp_ratio = 0.2, noise_scale = 0.5, noise_scale_w = 0.9, length_scale = 1):
    with torch.no_grad():
...
@app.route("/api/to_voice")
def to_voice ():
    text = request.args.get('text', '', str)
    format = request.args.get('format', 'mp3', str)
    ...
    sample_rate = 44100  # Adjust this to match your sample rate

    int16_array = (data * 32767).astype(np.int16)
    audio = AudioSegment(
        int16_array.tobytes(),
        frame_rate=sample_rate,
        sample_width=int16_array.dtype.itemsize,
        channels=1  # Adjust this for stereo audio
    )
    opus_raw = audio.export(format=format).read()

    res = make_response(opus_raw)
    return res

app.run(port=5001)

    # AudioSegment.converter = which("ffmpeg")
    # result = tts_fn('大家好，我是东雪莲。')
